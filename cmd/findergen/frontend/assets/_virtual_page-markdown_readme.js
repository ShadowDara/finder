...
<span class="hljs-punctuation">}</span></code></pre><p>A full template with all supported fields:</p><pre><code class="language-json5"><span class="hljs-punctuation">{</span>
  ...
  size<span class="hljs-punctuation">:</span> <span class="hljs-punctuation">{</span>
    ...
    min_size_type<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;KB&quot;</span><span class="hljs-punctuation">,</span>
  <span class="hljs-punctuation">}</span><span class="hljs-punctuation">,</span>
<span class="hljs-punctuation">}</span></code></pre><h3 id="custom-templates">Custom templates</h3><p>Place your own <code>.json5</code> template files in:</p><table><thead><tr><th>OS</th><th>Path</th></tr></thead><tbody><tr><td>Windows</td><td>\`%USERPROFILE%\\.finder\\templates\`</td></tr><tr><td>Linux</td><td><code>~/.finder/templates/</code></td></tr><tr><td>macOS</td><td><code>~/.finder/templates/</code></td></tr></tbody></table><p>Or in the project-local directory:</p><pre><code>./.finder/templates/</code></pre><p>User templates take precedence over built-in templates with the same name. See <a href="CUSTOM_TEMPLATES.md">\`CUSTOM_TEMPLATES.md\`</a> for the full guide.</p><h3 id="pattern-matching-exact-and-glob-regex-via-name_regex">Pattern matching (exact and glob); regex via <code>name_regex</code></h3><p>The <code>name</code> field (top-level, files, and folders) uses a **2-tier matching strategy** — exact match first, then glob (<code>path.Match</code>). Regular expressions are <strong>not</strong> interpreted inside <code>name</code> anymore.</p><table><thead><tr><th>Priority</th><th>Method</th><th>Applies when</th></tr></thead><tbody><tr><td>1</td><td>Exact string equality</td><td>pattern equals the name verbatim</td></tr><tr><td>2</td><td>Glob (<code>path.Match</code>)</td><td>pattern contains <code>*</code>/<code>?</code>/<code>[</code></td></tr></tbody></table><p>For regex matching use the separate <strong><code>name_regex</code></strong> field (available on top-level templates, files, and folders). It is matched with a full Go regex (RE2) against the name. When both <code>name</code> and <code>name_regex</code> are set, <strong>both</strong> must match.</p><p>This means:</p><ul><li><code>&quot;src&quot;</code> → exact match</li><li><code>&quot;*&quot;</code> → matches any name</li><li><code>&quot;*.ts&quot;</code> → glob, matches any <code>.ts</code> file</li><li><code>&quot;<sup>project-[0-9]+$&quot;</sup></code> in <code>name</code> → <strong>not</strong> a regex anymore; a <code></code>/<code>$</code></li></ul><p>Regex patterns live in the <code>name_regex</code> field:</p><pre><code class="language-json5"><span class="hljs-comment">// top-level name_regex: match folder names like project-42, project-99</span>
<span class="hljs-punctuation">{</span>
  name<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;*&quot;</span><span class="hljs-punctuation">,</span>
  name_regex<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;^project-[0-9]+$&quot;</span><span class="hljs-punctuation">,</span>
  min_version<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;0.3.18&quot;</span><span class="hljs-punctuation">,</span>
  files<span class="hljs-punctuation">:</span> <span class="hljs-punctuation">[</span>
    <span class="hljs-punctuation">{</span>
      <span class="hljs-comment">// file name_regex: match exactly main.py, app.py, or server.py</span>
      name_regex<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;^(main|app|server)\\\\.py$&quot;</span><span class="hljs-punctuation">,</span>
      existence<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;required&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-punctuation">}</span><span class="hljs-punctuation">,</span>
  <span class="hljs-punctuation">]</span><span class="hljs-punctuation">,</span>
  folders<span class="hljs-punctuation">:</span> <span class="hljs-punctuation">[</span>
    <span class="hljs-punctuation">{</span>
      <span class="hljs-comment">// folder name_regex: match src, lib, or pkg</span>
      name_regex<span class="hljs-punctuation">:</span> <span class="hljs-string">&quot;^(src|lib|pkg)$&quot;</span><span class="hljs-punctuation">,</span>
    <span class="hljs-punctuation">}</span><span class="hljs-punctuation">,</span>
  <span class="hljs-punctuation">]</span><span class="hljs-punctuation">,</span>
<span class="hljs-punctuation">}</span></code></pre><blockquote><p><strong>Note:</strong> Go regex is RE2 — no backreferences and no lookahead/lookbehind. An invalid regex never matches (it fails closed); use <code>finder check</code>/<code>finder validate</code> to catch mistakes.</p></blockquote><p>Templates that rely on <code>name_regex</code> should set <code>&quot;min_version&quot;: &quot;0.3.18&quot;</code>.</p><h2 id="config">Config</h2><p>Since <strong>v0.3.15</strong>, finder has a global config file at <code>~/.finder/config.json5</code>. If the file does not exist, defaults are used:</p><pre><code class="language-json5"><span class="hljs-punctuation">{</span>
