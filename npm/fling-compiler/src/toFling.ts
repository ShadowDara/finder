...
function assignStmt(name: string, value: FlingExpr): FlingStmt {
  return {
    ...
    expression: {
      type: "AssignmentExpr",
      target: { type: "Identifier", symbol: name },
      value,
    },
  };
}
...
export function convertBodyStatements(
  list: t.Statement[],
  diagnostics: DiagnosticBag,
): FlingStmt[] {
  const out: FlingStmt[] = [];
...
function convertStatement(
  ...
  switch (stmt.type) {
    ...
    case "TSModuleDeclaration":
      throw new UnsupportedError(
        "TypeScript namespaces/modules are not supported.",
        { loc: locOf(stmt) },
      );
    ...
    case "ThrowStatement":
      throw new UnsupportedError(
        "`throw` is not supported -- Fling has no exceptions.",
        { loc: locOf(stmt) },
      );
    ...
    case "LabeledStatement":
      throw new UnsupportedError("Labeled statements are not supported.", {
        loc: locOf(stmt),
      });
    ...
    default:
      throw new UnsupportedError(`Unsupported statement type: ${stmt.type}`, {
        loc: locOf(stmt),
      });
  }
}
...
function convertVariableDeclaration(
  ...
  for (const decl of node.declarations) {
    ...
    if (t.isConditionalExpression(init)) {
      ...
      out.push(
        ...lowerConditionalToIf(
          init,
          (leaf) => [assignStmt(name, convertExpr(leaf, diagnostics))],
          diagnostics,
        ),
      );
    } else {
...
function convertFunctionDeclaration(
  ...
  if (node.generator) {
    throw new UnsupportedError("Generator functions are not supported.", {
      loc: locOf(node),
    });
  }
  if (node.async) {
...
function convertIf(
  ...
  const thenBranch = convertBodyStatements(
    stmtsOf(node.consequent),
    diagnostics,
  );
  const elseBranch = node.alternate
    ? convertBodyStatements(stmtsOf(node.alternate), diagnostics)
    : null;
...
function convertFor(
  ...
  const updateStmts = node.update
    ? convertExpressionAsStatement(node.update, diagnostics)
    : [];

  return [
    ...initStmts,
    { type: "WhileStatement", condition, body: [...bodyStmts, ...updateStmts] },
  ];
}

function convertDoWhile(
...
function convertExpressionAsStatement(
  exprIn: t.Expression,
  diagnostics: DiagnosticBag,
): FlingStmt[] {
  ...
  if (t.isConditionalExpression(expr)) {
    return lowerConditionalToIf(
      expr,
      (leaf) => convertExpressionAsStatement(leaf, diagnostics),
      diagnostics,
    );
  }
  ...
  if (t.isSequenceExpression(expr)) {
    throw new UnsupportedError(
      "The comma operator (`a, b, c`) is not supported.",
      { loc: locOf(expr) },
    );
  }
  ...
  if (t.isUpdateExpression(expr)) {
    if (!t.isIdentifier(expr.argument)) {
      throw new UnsupportedError(
        "`++`/`--` is only supported on a plain variable.",
        { loc: locOf(expr) },
      );
    }
    const target: Identifier = {
      type: "Identifier",
      symbol: expr.argument.name,
    };
    ...
    return [
      {
        type: "ExpressionStatement",
        expression: { type: "AssignmentExpr", target, value },
      },
    ];
  }
  ...
  if (t.isAssignmentExpression(expr) && expr.operator === "=") {
    if (t.isObjectPattern(expr.left) || t.isArrayPattern(expr.left)) {
      throw new UnsupportedError("Destructuring assignment is not supported.", {
        loc: locOf(expr),
      });
    }
  ...
  return [
    { type: "ExpressionStatement", expression: convertExpr(expr, diagnostics) },
  ];
}
...
function convertIdentifier(
  ...
  if (node.name === "undefined") {
    throw new UnsupportedError(
      "`undefined` does not exist in Fling -- use `null` instead.",
      { loc: locOf(node) },
    );
  }
  if (node.name === "NaN" || node.name === "Infinity") {
    throw new UnsupportedError(`\`${node.name}\` does not exist in Fling.`, {
      loc: locOf(node),
    });
  }
...
function convertArray(
  ...
  for (const el of node.elements) {
    if (el === null) {
      throw new UnsupportedError(
        "Sparse array elements (holes, e.g. `[1, , 3]`) are not supported.",
        {
          loc: locOf(node),
        },
      );
    ...
    if (t.isSpreadElement(el)) {
      throw new UnsupportedError(
        "Spread syntax (`...arr`) is not supported -- Fling has no spread/rest (spec §10).",
        {
          loc: locOf(el),
        },
      );
...
function convertObject(
  ...
  for (const prop of node.properties) {
    if (t.isSpreadElement(prop)) {
      throw new UnsupportedError("Object spread (`...obj`) is not supported.", {
        loc: locOf(prop),
      });
    }
    ...
    if (t.isIdentifier(prop.key)) {
      ...
      /^[A-Za-z][A-Za-z0-9]*$/.test(prop.key.value)
    ) {
      key = prop.key.value;
    } else {
      throw new UnsupportedError(
    ...
    if (prop.shorthand) {
      ...
      throw new UnsupportedError("Unsupported object property value.", {
        loc: locOf(prop),
      });
    }
  }
...
function convertBinary(
  ...
  if (!t.isExpression(node.left) || !t.isExpression(node.right)) {
    throw new UnsupportedError("Unsupported binary operand.", {
      loc: locOf(node),
    });
  }
  ...
  if (
    mapped === "+" &&
    (looksLikeString(node.left) || looksLikeString(node.right))
  ) {
...
function convertLogical(
  ...
  if (node.operator === "??") {
    throw new UnsupportedError("Nullish coalescing (`??`) is not supported.", {
      loc: locOf(node),
    });
  }
  diagnostics.warn(
...
function convertUnary(
  node: t.UnaryExpression,
  diagnostics: DiagnosticBag,
): FlingExpr {
  if (!t.isExpression(node.argument)) {
    throw new UnsupportedError("Unsupported unary operand.", {
      loc: locOf(node),
    });
  }
  if (node.operator === "!") {
    return {
      type: "UnaryExpr",
      operator: "!",
      operand: convertExpr(node.argument, diagnostics),
    };
  }
  if (node.operator === "-") {
    return {
...
function convertAssignment(
  ...
  if (node.operator === "=") {
    ...
    return {
      type: "AssignmentExpr",
      target,
      value: convertExpr(rhs, diagnostics),
    };
  }
  ...
  return {
    type: "AssignmentExpr",
    target,
    value: { type: "BinaryExpr", operator: mapped, left, right },
  };
}
...
function convertCall(
  ...
  if (node.optional) {
    throw new UnsupportedError("Optional call (`fn?.()`) is not supported.", {
      loc: locOf(node),
    });
  }
  if (!t.isExpression(node.callee)) {
    throw new UnsupportedError("Unsupported call target (e.g. `super(...)`).", {
      loc: locOf(node),
    });
  }
  ...
  if (isConsoleCall || isPrintCall) {
    return {
      type: "CallExpr",
      callee: { type: "Identifier", symbol: "print" },
      args: expandPrintArgs(node.arguments, diagnostics),
    };
  }
  ...
  for (const a of node.arguments) {
    if (t.isSpreadElement(a)) {
      throw new UnsupportedError(
        "Spread arguments (`...args`) are not supported.",
        { loc: locOf(a) },
      );
    }
    if (!t.isExpression(a)) {
      throw new UnsupportedError("Unsupported call argument.", {
        loc: locOf(a),
      });
    }
    const ua = unwrapTS(a);
...
function interleaveTemplate(node: t.TemplateLiteral): TemplatePart[] {
  ...
  node.quasis.forEach((q: t.TemplateElement, i: number) => {
    ...
    if (i < node.expressions.length) {
      ...
      if (!t.isExpression(e)) {
        throw new UnsupportedError(
          "Unsupported template literal interpolation.",
          { loc: locOf(node) },
        );
      }
...
function expandPrintArgs(
  ...
  for (const a of rawArgs) {
    if (t.isSpreadElement(a)) {
      throw new UnsupportedError(
        "Spread arguments to `print` are not supported.",
        { loc: locOf(a) },
      );
    }
...
function convertMember(
  ...
  if (node.optional) {
    throw new UnsupportedError("Optional chaining (`?.`) is not supported.", {
      loc: locOf(node),
    });
  }
  if (t.isSuper(node.object)) {
    throw new UnsupportedError(
      "`super` is not supported (no classes/inheritance in Fling).",
      { loc: locOf(node) },
    );
  }
  ...
  if (node.computed) {
    ...
    if (!t.isExpression(node.property)) {
      throw new UnsupportedError("Unsupported computed member property.", {
        loc: locOf(node),
      });
    }
  ...
  if (!t.isIdentifier(node.property)) {
    throw new UnsupportedError("Unsupported member property.", {
      loc: locOf(node),
    });
  }
