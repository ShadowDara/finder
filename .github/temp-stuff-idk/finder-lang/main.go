...
type Template struct {
	...
	// Query is the new field: a small boolean expression language (see
	// package "fincheck/query") that lets a template express match
	// conditions the structured files/folders schema cannot - OR, NOT,
	// grouping, and count/size thresholds. Optional; when present it is
	// evaluated in addition to (ANDed with) every other required
	// condition. Since 0.4.0 (proposed).
...
func Validate(t Template) []Issue {
	...
	if len(t.Files) == 0 && len(t.Folders) == 0 && t.Command == "" && strings.TrimSpace(t.Query) == "" {
		issues = append(issues, Issue{SevWarn, "", "weder files noch folders noch command noch query definiert - Template matcht praktisch jeden Ordner"})
	}
	...
	needsQueryVersion := false
	if strings.TrimSpace(t.Query) != "" {
		if _, err := query.Compile(t.Query); err != nil {
			issues = append(issues, Issue{SevError, "query", fmt.Sprintf("Syntaxfehler: %v", err)})
		} else {
			needsQueryVersion = true
		}
	}
	...
	if needsQueryVersion {
		required = maxVersion(required, "0.4.0")
		reasons = append(reasons, "query verwendet")
	}
	if required != "0.0.0" {
		if t.MinVersion == "" {
...
func renderTemplate(t Template) (string, error) {
	...
	enc.SetEscapeHTML(false) // sonst würde "&&" in query-Strings zu \u0026\u0026 werden
	enc.SetIndent("", "  ")
	if err := enc.Encode(t); err != nil {
		return "", err
	}
	return string(toJSON5Style([]byte(strings.TrimRight(buf.String(), "\n")))) + "\n", nil
}

// ---------------------------------------------------------------------
// 5.5) Vollständiges Matching gegen ein echtes Verzeichnis (inkl. query)
// ---------------------------------------------------------------------
...
func checkFileEntryOnDisk(ctx *query.FSContext, f FileEntry, report *MatchReport) {
	exists, err := ctx.MatchFile(f.Name)
	if err != nil {
		report.Lines = append(report.Lines, fmt.Sprintf("  FEHLER bei files[%q]: %v", f.Name, err))
	...
	report.Lines = append(report.Lines, fmt.Sprintf("  [%s] files[%q] existence=%s", mark, f.Name, existence))
}

func checkFolderOnDisk(baseDir string, fo Folder, report *MatchReport) {
	dirCtx := query.NewFSContext(baseDir)
	exists, err := dirCtx.MatchFolder(fo.Name)
	if err != nil {
		report.Lines = append(report.Lines, fmt.Sprintf("  FEHLER bei folders[%q]: %v", fo.Name, err))
		report.Matches = false
		return
	}
	if !exists {
		report.Lines = append(report.Lines, fmt.Sprintf("  [FEHLT] folders[%q]", fo.Name))
		report.Matches = false
		return
	}
	report.Lines = append(report.Lines, fmt.Sprintf("  [OK] folders[%q]", fo.Name))
	// find the actual matching subfolder to recurse into
	entries, _ := os.ReadDir(baseDir)
	for _, e := range entries {
		if e.IsDir() && query.MatchPattern(fo.Name, e.Name()) {
			sub := baseDir + string(os.PathSeparator) + e.Name()
			subCtx := query.NewFSContext(sub)
			for _, f := range fo.Files {
				checkFileEntryOnDisk(subCtx, f, report)
			}
			for _, subFo := range fo.Folders {
				checkFolderOnDisk(sub, subFo, report)
			}
			break
		}
	}
...
// MatchDirectory checks whether dir matches template t: name, files,
// folders (recursively), size and - the new part - the query expression.
func MatchDirectory(t Template, dir string) MatchReport {
	report := MatchReport{Matches: true}
	base := dir
	if idx := strings.LastIndexAny(dir, `/\`); idx >= 0 {
		base = dir[idx+1:]
	}

	nameOK := t.Name == "" || query.MatchPattern(t.Name, base)
	mark := "OK"
	if !nameOK {
		mark = "FEHLT"
		report.Matches = false
	}
	report.Lines = append(report.Lines, fmt.Sprintf("[%s] name %q gegen Ordnername %q", mark, t.Name, base))

	ctx := query.NewFSContext(dir)
	for _, f := range t.Files {
		checkFileEntryOnDisk(ctx, f, &report)
	}
	for _, fo := range t.Folders {
		checkFolderOnDisk(dir, fo, &report)
	}

	if t.Size != nil {
		total, err := ctx.DirSizeBytes()
		if err != nil {
			report.Lines = append(report.Lines, fmt.Sprintf("[FEHLER] size: %v", err))
			report.Matches = false
		} else {
			minStr, maxStr := "", ""
			if t.Size.Min != nil {
				minStr = fmt.Sprintf("%g%s", *t.Size.Min, t.Size.MinSizeType)
			}
			if t.Size.Max != nil {
				maxStr = fmt.Sprintf("%g%s", *t.Size.Max, t.Size.MaxSizeType)
			}
			ok, err := func() (bool, error) {
				// reuse the query package's bound helper via a tiny call node
				n := &query.CallNode{Name: "size", Args: []string{minStr, maxStr}}
				return query.Eval(n, ctx)
			}()
			if err != nil {
				report.Lines = append(report.Lines, fmt.Sprintf("[FEHLER] size: %v", err))
				report.Matches = false
			} else if !ok {
				report.Lines = append(report.Lines, fmt.Sprintf("[FEHLT] size außerhalb [%s, %s] (tatsächlich %d Bytes)", minStr, maxStr, total))
				report.Matches = false
			} else {
				report.Lines = append(report.Lines, fmt.Sprintf("[OK] size innerhalb [%s, %s] (tatsächlich %d Bytes)", minStr, maxStr, total))
			}
		}
	}

	if strings.TrimSpace(t.Query) != "" {
		ast, err := query.Compile(t.Query)
		if err != nil {
			report.Lines = append(report.Lines, fmt.Sprintf("[FEHLER] query: %v", err))
			report.Matches = false
		} else {
			ok, err := query.Eval(ast, ctx)
			if err != nil {
				report.Lines = append(report.Lines, fmt.Sprintf("[FEHLER] query zur Laufzeit: %v", err))
				report.Matches = false
			} else if !ok {
				report.Lines = append(report.Lines, fmt.Sprintf("[FEHLT] query: %s", t.Query))
				report.Matches = false
			} else {
				report.Lines = append(report.Lines, fmt.Sprintf("[OK] query: %s", t.Query))
			}
		}
	}
...
func usage() {
	fmt.Fprint(os.Stderr, `fincheck - Finder-Template Checker/Completer

Verwendung:
  fincheck check <template.json5>
  fincheck complete <template.json5> [-out out.json5] [-write]
  fincheck eval <template.json5> <verzeichnis>
  ...
  eval      : matcht das Template komplett gegen ein echtes Verzeichnis
              (name, files, folders, size UND das neue query-Feld) und
              zeigt Zeile für Zeile, was gepasst hat
`)
}
...
func main() {
	...
	case "eval":
		...
		if err != nil || !info.IsDir() {
			fmt.Fprintf(os.Stderr, "[%s] %s ist kein gültiges Verzeichnis\n", SevError, dir)
			os.Exit(1)
		}
		report := MatchDirectory(t, dir)
