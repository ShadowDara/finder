...
function usage(): never {
  console.error("Usage: flingc <input.ts|input.js> [-o output.fling] [--warnings-as-errors]");
  process.exit(2);
}
...
function main() {
  ...
  const errorCount = result.diagnostics.items.filter((d) => d.severity === "error").length;
  const warnCount = result.diagnostics.items.filter((d) => d.severity === "warning").length;
  ...
  if (failed) {
    console.error(`Compilation failed: ${errorCount} error(s), ${warnCount} warning(s).`);
    process.exit(1);
  }
